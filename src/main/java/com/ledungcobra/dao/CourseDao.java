...
public class CourseDao extends BaseDao<Course, String> {
    ...
    public List<Course> search(String text) {
        ...
        String extension = "";
        if (text.matches("\\d+")) {
            extension = " or c.numberOfSlot = :slot or c.credit = :credit";

        }

        try {
            val query = session.createQuery("select distinct  c from Course c" +
                    " where c.id like :keyword or c.subject.name like :keyword or c.teacherName like :keyword" +
                    " or c.dayToStudyInWeek like :keyword or c.classroomName like :keyword or c.shiftToStudyInDay like :keyword" + extension);
            if (text.matches("\\d+")) {
